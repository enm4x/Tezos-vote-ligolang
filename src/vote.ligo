const owner : address = ("tz1Yzb54tZxbDDEePxKPPCCV4H2TiN667row" : address);
...
function pause_vote (const stat : string; const store : storage) : storage is
 block { 
   ...
   else if Tezos.source = owner and stat = "True"
      then 
        if stat = "True" then
          store.status := True
...
block {
    ...
    if Tezos.source = owner and store.status = False then 
